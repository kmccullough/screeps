...
_.extend(Creep.prototype, {
    ...
    // Run creep's assigned task
    run: function() {

        let creep = this;

        // Follow aliases until a role is found, or use default
        let defaultRole = config.$('Creep.defaultRole');
        let role = creep.memory.role;
        while (!role || _.isString(role)) {
            role = creep.roles[role || defaultRole];
        }
    ...
    setTask: function(taskName) {
        ...
        if (task) {
            if (creep.memory.task) {
                ...
                console.log(`${creep.memory.role} ${creep} went from idling to ${task.verb}`);
            }
