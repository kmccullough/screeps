    ...
    setTask: function(taskName) {
        ...
        if (task) {
            if (creep.memory.task) {
                creep.say(`${creep.tasks[creep.memory.task].short}${arrow}${task.short}`);
                console.log(`${creep.memory.role} ${creep} went from ${creep.tasks[creep.memory.task].verb} to ${task.verb}`);
            } else {
